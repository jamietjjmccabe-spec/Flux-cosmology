...
def compute_planetary_battery(row: pd.Series) -> Dict[str, float]:
    ...
    battery_coherence_score = clamp01(0.25 * ecem_flux + 0.35 * ecee + 0.40 * battery)
    return {
...
def classify(row: pd.Series) -> str:
    ...
    rocky = finite_or_nan(row.get("score_ecee_rocky_boundary")); battery = finite_or_nan(row.get("planetary_battery_score"))
    ...
    if star_class in ["G", "K"] and np.isfinite(r) and 1.2 <= r <= 2.0 and np.isfinite(battery) and battery > 0.70 and np.isfinite(core_star) and core_star > 1.0:
        return "outer super-Earth planetary-battery candidate"
    if star_class == "K" and np.isfinite(battery) and battery > 0.65:
        return "K-dwarf long-coherence battery branch"
    if star_class == "M" and np.isfinite(tidal) and tidal > 100 and np.isfinite(battery) and battery > 0.55:
        return "dense M-dwarf retained-battery branch"
    if star_class == "M" and np.isfinite(tidal) and tidal > 100:
        return "compact M-dwarf high-tidal branch"
    if np.isfinite(ecee) and ecee > 0.60 and np.isfinite(battery) and battery > 0.60:
        return "non-Earth coherent envelope candidate"
...
def score_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    outdf["rocky_boundary_gate"] = outdf["score_battery_rocky_boundary"].clip(0, 1)
    outdf["usable_battery_score"] = outdf["planetary_battery_score"].clip(0, 1) * outdf["rocky_boundary_gate"]
    outdf["battery_coherence_usable_score"] = (
...
def save_plots(df: pd.DataFrame, outdir: Path, prefix: str, top: int) -> None:
    ...
    plt.barh(y + 0.25, topdf["planetary_battery_score"], height=0.24, label="Planetary battery")
    plt.yticks(y, topdf["name"])
    plt.xlabel("score")
    plt.title("ECEM vs ECEE vs planetary battery")
    ...
    plt.scatter(df["core_to_stellar_ratio_relative"], df["planetary_battery_score"], s=80)
    for _, r in topdf.iterrows():
        plt.annotate(str(r["name"]), (r["core_to_stellar_ratio_relative"], r["planetary_battery_score"]), fontsize=8, xytext=(4,4), textcoords="offset points")
    plt.xscale("log")
    plt.xlabel("core-to-stellar flux ratio / Earth")
    plt.ylabel("planetary_battery_score")
    ...
    plt.figure(figsize=(8, 6))
    plt.scatter(df["ECEE_score"], df["planetary_battery_score"], s=80)
    for _, r in topdf.iterrows():
        plt.annotate(str(r["name"]), (r["ECEE_score"], r["planetary_battery_score"]), fontsize=8, xytext=(4,4), textcoords="offset points")
    ...
    plt.figure(figsize=(8, 6))
    sc = plt.scatter(df["planet_radius_earth"], df["planet_mass_earth"], c=df["planetary_battery_score"], s=90)
    plt.colorbar(sc, label="planetary_battery_score")
    for _, r in topdf.iterrows():
    ...
    plt.title("Dense rocky/super-Earth battery candidates vs wrong-boundary controls")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
